   ...
   //converts string from config to code
      ...
      public static void loadConfig() {
         ...
         for(int x = -1; x < configMonths.size(); x++)
            LPluginHelper.runMonth.put(x, new ArrayList<Runnable>());
         if(debug) { LLogger.info("Set up Monthly Plugin List."); } 
         ...
         for(int x = -1; x < configDaysInAMonth; x++)
            LPluginHelper.runDay.put(x, new ArrayList<Runnable>());
         if(debug) { LLogger.info("Set up Daily Plugin List."); }
